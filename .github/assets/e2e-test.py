...
def verify_go_argon2_pw(go_argon2, password):
    def argon2_hex_to_b64(hex):
        return base64.encodebytes(bytes.fromhex(hex)).decode('utf-8').strip().replace('=', '')
    argon2_split = go_argon2.split('$')[1:]

    parameter = {x.split('=')[0]: int(x.split('=')[1])
                 for x in argon2_split[2].split(',')}
    salt = argon2_hex_to_b64(argon2_split[3])
    hash = argon2_hex_to_b64(argon2_split[4])

    ph = PasswordHasher(time_cost=parameter['t'], memory_cost=parameter['m'], parallelism=parameter['p'], hash_len=int(
        len(argon2_split[4])/2), salt_len=int(len(argon2_split[3])/2))
...
class FileServerTest(unittest.TestCase):
    ...
    def test_upload(self):
        ...
        with open(file, 'rb') as f:
            files = {'file': f}
            response = requests.put(upload_url, files=files, auth=self.req_auth, verify=False)
